...
do
  cp record_result.php ${example};
done

for example in datasaving;
do
  cp record_result_incremental.php ${example};
done

wget https://github.com/jspsych/jsPsych/releases/download/v6.3.1/jspsych-6.3.1.zip
unzip jspsych-6.3.1.zip
for example in s02 s04 s05 datasaving factorial_with_fixation factorial;
do
...
unzip images.zip
for example in datasaving factorial_with_fixation factorial;
do
  cp images/* ${example};
done
